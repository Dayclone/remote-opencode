...
export const session: Command = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName('list').setDescription('List available sessions for the bound project'),
    )
    .addSubcommand((subcommand) =>
      subcommand.setName('attach').setDescription('Attach this thread to an existing session'),
    )
    .addSubcommand((subcommand) =>
      subcommand.setName('detach').setDescription('Detach the current session from this thread'),
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName('info')
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (subcommand === 'info') {
      ...
      try {
        const parentChannelId = getParentChannelId(interaction);
        const projectPath =
          dataStore.getChannelProjectPath(parentChannelId) ?? currentSession.projectPath;
