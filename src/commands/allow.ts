  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addUserOption((opt) =>
          opt.setName('user').setDescription('The user to allow').setRequired(true),
        ),
    )
    .addSubcommand((sub) =>
      sub
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (currentList.length === 0) {
      await interaction.reply({
        content:
          '⚠️ No allowlist configured. Use `remote-opencode allow add <userId>` or `remote-opencode setup` to set up access control first.',
