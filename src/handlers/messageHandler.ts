...
async function safeReact(message: Message, emoji: string): Promise<void> {
  try {
    ...
    console.error(
      `[Voice STT] Failed to react with ${emoji}:`,
...
async function safeRemoveReaction(message: Message, emoji: string): Promise<void> {
  try {
    ...
    console.error(
      `[Voice STT] Failed to remove reaction ${emoji}:`,
      error instanceof Error ? error.message : error,
    );
  }
}
...
export async function handleMessageCreate(message: Message): Promise<void> {
  ...
  const isVoiceMessage =
    !prompt && isVoiceEnabled() && message.flags.has(MessageFlags.IsVoiceMessage);
  ...
  if (voiceAttachment) {
    ...
    try {
      ...
      console.error(
        '[Voice STT] Transcription failed:',
        error instanceof Error ? error.message : error,
      );
      await safeReact(message, '❌');
      if (error instanceof Error && error.message === 'AUTH_FAILURE') {
        await message
          .reply({
            content: '❌ Transcription failed. Please check your API key with `/voice status`.',
          })
          .catch(() => {});
      } else {
        await message
          .reply({ content: '❌ Voice transcription failed. Check server logs for details.' })
          .catch(() => {});
      }
      return;
    }
